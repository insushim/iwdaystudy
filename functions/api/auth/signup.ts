...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    if (password.length < 4) {
      return jsonResponse({ message: '비밀번호는 최소 4자 이상이어야 합니다.' }, 400);
    }
    ...
    await context.env.DB.prepare(
      `INSERT INTO profiles (id, email, name, role, grade, semester, school_name, password_hash, subscription_plan, streak_count, total_points, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'free', 0, 0, ?, ?)`
    ).bind(
      id, email, name, role,
      grade ?? null,
    ...
    return jsonResponse({ message: err.message || '회원가입 처리 중 오류가 발생했습니다.' }, 500);
  }
};
