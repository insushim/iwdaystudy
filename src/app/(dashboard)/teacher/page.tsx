...
// Mock data
const classOverview = [
  { id: "c1", name: "3학년 1반", students: 28, completionRate: 82, avgScore: 78 },
  { id: "c2", name: "3학년 2반", students: 26, completionRate: 75, avgScore: 81 },
];
...
const recentActivity = [
  { id: "a1", text: "김민준 학생이 오늘의 학습을 완료했습니다. (95점)", time: "5분 전", type: "complete" },
  { id: "a2", text: "이서연 학생이 7일 연속 뱃지를 획득했습니다!", time: "12분 전", type: "badge" },
  { id: "a3", text: "3학년 1반 주간 완료율이 80%를 넘었습니다.", time: "30분 전", type: "milestone" },
  { id: "a4", text: "강하은 학생이 수학에서 만점을 받았습니다!", time: "45분 전", type: "perfect" },
  { id: "a5", text: "박지호 학생이 학습을 시작했습니다.", time: "1시간 전", type: "start" },
];
...
const STATUS_CONFIG = {
  completed: { label: "완료", color: "bg-emerald-500", badge: "default" as const },
  in_progress: { label: "진행 중", color: "bg-amber-500", badge: "secondary" as const },
  not_started: { label: "미시작", color: "bg-gray-300", badge: "outline" as const },
};

export default function TeacherDashboardPage() {
  const totalStudents = classOverview.reduce((sum, c) => sum + c.students, 0);
  const todayCompleted = todayStudents.filter((s) => s.status === "completed").length;
  const avgCompletion = Math.round(
    classOverview.reduce((sum, c) => sum + c.completionRate, 0) / classOverview.length
  );
  const avgScore = Math.round(
    classOverview.reduce((sum, c) => sum + c.avgScore, 0) / classOverview.length
  );
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-primary">{cls.avgScore}점</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <ScrollArea className="h-[320px]">
                    <div className="space-y-3">
                      {todayStudents.map((student) => {
                        const config = STATUS_CONFIG[student.status as keyof typeof STATUS_CONFIG];
                        return (
                          <div
                            key={student.id}
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant={config.badge} className="text-xs">
                                <span className={`h-1.5 w-1.5 rounded-full ${config.color} mr-1`} />
                                {config.label}
                              </Badge>
                            </div>
                          </div>
