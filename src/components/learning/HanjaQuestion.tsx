...
export default function HanjaQuestion({
  ...
  // ── 결과 뱃지 ──
  const ResultBadge = ({ correct }: { correct: string }) => (
    <motion.div
      initial={{ scale: 0.8 }}
      animate={{ scale: 1 }}
  ...
  if (variant === "reading") {
    return (
      <div className="flex flex-col items-center gap-6">
        ...
        <WordBadges />
        <ChoiceGrid items={choices} correct={correctAnswer} />
      </div>
    );
  }
      ...
      <div className="flex flex-col items-center gap-6">
        ...
        <AnimatePresence>
          {showResult && (
            ...
            >
              <ResultBadge correct={correctAnswer} />
