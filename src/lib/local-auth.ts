 ...
 * Throws if email/password do not match.
 */
 ...
 */
export function localUpdateProfile(userId: string, updates: Partial<Profile>): Profile {
  const users = getStoredUsers();
...
export function shouldUseLocalAuth(): boolean {
  ...
  const useApi = typeof process !== 'undefined'
 ...
 * Creates sample student, teacher, and parent accounts if none exist.
 */
