...
export function getSubjectStats(
  ...
  for (const resp of allResponses) {
    ...
    const subj = question.subject as string;
...
/**
 * Seed realistic demo learning data for a student.
 * Safe to call multiple times — replaces existing demo data only.
 */
export function seedStudentDemoData(
  studentId: string,
  grade: number,
  semester: number,
): void {
  ...
  const subjectDist: { subject: SubjectType; count: number }[] =
    grade <= 2
      ? [
          { subject: 'math', count: 3 },
          { subject: 'spelling', count: 2 },
          { subject: 'vocabulary', count: 2 },
          { subject: 'creative', count: 1 },
        ]
      : grade <= 4
        ? [
            { subject: 'math', count: 3 },
            { subject: 'spelling', count: 2 },
            { subject: 'vocabulary', count: 1 },
            { subject: 'hanja', count: 2 },
            { subject: 'creative', count: 1 },
          ]
        : [
            { subject: 'math', count: 2 },
            { subject: 'spelling', count: 1 },
            { subject: 'vocabulary', count: 1 },
            { subject: 'hanja', count: 1 },
            { subject: 'english', count: 2 },
            { subject: 'science', count: 1 },
            { subject: 'creative', count: 1 },
          ];

  const gradedSubjects = subjectDist.filter((s) => s.subject !== 'creative');
  const totalQPerSession = subjectDist.reduce((a, s) => a + s.count, 0);
  const maxScore = totalQPerSession * 10;

  // Subject-specific accuracy modifier
  const subjectMod: Partial<Record<SubjectType, number>> = {
    math: -0.05,
    spelling: 0,
    vocabulary: 0.05,
    hanja: -0.1,
  ...
  // Build list of session dates (last 65 days, Mon–Sat, ~80% completion)
  const now = new Date();
  ...
  for (let i = 65; i >= 1; i--) {
    ...
    const rng = mkRng(i * 31 + grade * 7 + semester * 3);
    if (dow === 6 && rng() > 0.45) continue; // Sat: 45% chance
    if (dow !== 6 && rng() > 0.85) continue; // Weekday: 85% chance
    sessionDates.push(new Date(d));
  }
  // Keep latest 45 sessions
  const dates = sessionDates.slice(-45);
  const total = dates.length;

  const newSets: DailySet[] = [];
  const newQuestions: Question[] = [];
  const newRecords: LearningRecord[] = [];
  const newResponses: QuestionResponse[] = [];

  dates.forEach((date, idx) => {
    const rng = mkRng(idx * 97 + grade * 13 + semester * 5 + 42);
    const dateStr = `${date.getFullYear()}${String(date.getMonth() + 1).padStart(2, '0')}${String(date.getDate()).padStart(2, '0')}`;
    const setId = `demo-${grade}-${semester}-${dateStr}`;
    const recordId = `demo-rec-${dateStr}`;

    // Improvement arc: 62% → 90%
    const progress = idx / Math.max(total - 1, 1);
    const baseAcc = 0.62 + progress * 0.28;
    const sessionAcc = Math.min(0.97, Math.max(0.4, baseAcc + (rng() - 0.5) * 0.12));

    // Session timing: 7:20–8:50 KST
    const hour = 7 + Math.floor(rng() * 1.5);
    const min = Math.floor(rng() * 60);
    date.setHours(hour, min, 0, 0);
    const sessionISO = date.toISOString();
    const timeSeconds = Math.round(1200 + rng() * 900); // 20–35 min
    const completedISO = new Date(date.getTime() + timeSeconds * 1000).toISOString();
    ...
    newSets.push({
      ...
      semester,
      set_number: idx + 1,
      title: `${grade}학년 ${semester}학기 #${idx + 1}`,
      description: '오늘의 아침학습',
      estimated_minutes: 30,
      total_questions: totalQPerSession,
    ...
    // Questions
    const qs: Question[] = [];
    // emotion_check (subject stored as 'emotion_check' cast)
    qs.push({
      id: `${setId}-q-emotion`,
      daily_set_id: setId,
      curriculum_standard_id: null,
      subject: 'emotion_check' as SubjectType,
      question_type: 'emotion_check',
      order_index: 0,
      title: null,
      content: {},
      answer: {},
    ...
    let order = 1;
    for (const { subject, count } of subjectDist) {
      for (let qi = 0; qi < count; qi++) {
        qs.push({
          id: `${setId}-q-${subject}-${qi}`,
          daily_set_id: setId,
          curriculum_standard_id: null,
          subject,
          question_type: subject === 'creative' ? 'short_answer' : 'multiple_choice',
          order_index: order++,
          title: null,
          content: {},
          answer: {},
    ...
    // Responses + scores
    let totalScore = 0;
    const timePerQ = Math.round(timeSeconds / qs.length);
    for (const q of qs) {
      const subj = q.subject as string;
      if (subj === 'emotion_check' || subj === 'readiness_check') {
        newResponses.push({
          id: `${recordId}-${q.id}`,
      ...
      if (subj === 'creative') {
        newResponses.push({
          ...
          learning_record_id: recordId,
          question_id: q.id,
          student_answer: JSON.stringify('창의적인 답변'),
          is_correct: true,
          score: 10,
      ...
      const mod = subjectMod[q.subject] ?? 0;
      const acc = Math.min(0.97, Math.max(0.3, sessionAcc + mod));
      ...
      newResponses.push({
        ...
        learning_record_id: recordId,
        question_id: q.id,
        student_answer: JSON.stringify(correct ? '정답' : '오답'),
        is_correct: correct,
        score,
        time_spent_seconds: timePerQ,
        attempts: 1,
  ...
  // Merge: remove old demo data, add new
  const demoPfx = `demo-${grade}-${semester}-`;
  const existingSets = getList<DailySet>(DAILY_SETS_KEY).filter((s) => !s.id.startsWith(demoPfx));
  const demoSetIds = new Set(newSets.map((s) => s.id));
  const existingQs = getList<Question>(QUESTIONS_KEY).filter((q) => !demoSetIds.has(q.daily_set_id));
  const existingRecs = getList<LearningRecord>(RECORDS_KEY).filter((r) => !r.id.startsWith('demo-rec-'));
  const existingResps = getList<QuestionResponse>(RESPONSES_KEY).filter((r) => !r.id.startsWith('demo-rec-'));

  saveList(DAILY_SETS_KEY, [...existingSets, ...newSets]);
  saveList(QUESTIONS_KEY, [...existingQs, ...newQuestions]);
  saveList(RECORDS_KEY, [...existingRecs, ...newRecords]);
  saveList(RESPONSES_KEY, [...existingResps, ...newResponses]);

  // Award badges based on seeded data
